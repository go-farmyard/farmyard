...
func (c *instanceContainer) RegisterInstance(inst any, optionalName ...string) {
	...
	c.instanceTypeMetaMap[instTypeStr] = &instanceTypeMeta{managedFields: map[string]int{}}
	managedFieldStarted := false
	instStructType := instVal.Elem().Type()
	typeAutoInstantiate := reflect.TypeOf(AutoInstantiate{})
	typeCustomInstantiate := reflect.TypeOf(CustomInstantiate{})
	for i := 0; i < instStructType.NumField(); i++ {
		field := instStructType.Field(i)
		fieldName := field.Name
		if !managedFieldStarted {
			managedFieldStarted = field.Type == typeAutoInstantiate || field.Type == typeCustomInstantiate
			continue
		}
		c.instanceTypeMetaMap[instTypeStr].managedFields[fieldName] = i
	}

	if instantiatable, ok := instVal.Interface().(Instantiatable); ok {
		c.instancesPendingInit = append(c.instancesPendingInit, instantiatable)
	}
}
