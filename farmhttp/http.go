...
import (
	"context"
	"embed"
	"github.com/go-farmyard/farmyard/farmlog"
	"github.com/go-farmyard/farmyard/farmutil"
...
func (hs *HttpServer) initAssetsDir() {
	...
	if hs.devMode {
		...
		hs.AssetsDir, err = fs.Sub(hs.embedAssets, "assets")
		if err != nil {
			farmlog.Fatalf("can not open web directory for assets, err:%v", err)
		}
	}

	hs.AssetsWebRoot, err = fs.Sub(hs.AssetsDir, "web")
	if err != nil {
		farmlog.Fatalf("can not open web directory for http server, err:%v", err)
	}

	assetsTmplRoot, err := fs.Sub(hs.AssetsDir, "template")
	if err != nil {
		farmlog.Fatalf("can not open template directory for http server, err:%v", err)
	}
...
func (hs *HttpServer) wrapHandlers(handlers ...AnyHandler) http.HandlerFunc {
	...
	return func(wOrig http.ResponseWriter, rOrig *http.Request) {
		...
		if resp != nil && resp != responseNop {
			...
			if err != nil {
				farmlog.Infof("ERROR: failed to write response %T, err:%v", resp, err)
			}
		}

		statusCodeStr := "000"
		if ctx.IsResponseWritten() {
			statusCodeInt := ctx.ResponseWriter.statusCode
			if statusCodeInt == 0 {
				statusCodeInt = 200
			}
			if ctx.ResponseWriter.statusCode == 200 {
				statusCodeStr = "200"
			} else {
				statusCodeStr = strconv.Itoa(statusCodeInt)
			}
		}

		farmlog.Infof("httpweb: completed %s %s %s", statusCodeStr, r.Method, r.RequestURI)
		if !ctx.IsResponseWritten() {
			farmlog.Infof("httpweb: no response for %s %s", r.Method, r.RequestURI)
		}
	}
}
