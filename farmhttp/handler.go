...
type RequestHandler interface {
	HandlerFunc(c *Context) Response
}
...
func NewHandlerCaller(p AnyHandler) *HandlerCaller {
	...
	hc.pt = hc.pv.Type()
	farmutil.MustTrue(hc.pt.Kind() == reflect.Func, "handler must be a func, but: %T", p)
	hc.numIn = hc.pt.NumIn()
	hc.numOut = hc.pt.NumOut()
	farmutil.MustTrue(1 <= hc.numIn && hc.numIn <= 3, "handler must be: func([*ChainExecutor], [*XxxRequest] ...) Response")
	farmutil.MustTrue(hc.pt.NumOut() <= 1, "handler must be: func([*ChainExecutor], [*XxxRequest] ...) Response")
...
func (hc *HandlerCaller) Call(ce *ChainExecutor, isMiddleware bool) Response {
	...
	for i := 0; i < hc.numIn; i++ {
		...
		default:
			farmutil.Panic("unsupported handler argument type: %T", hc.argTypes[i])
		}
	}
	if isMiddleware != hasMiddleArg {
		farmutil.Panic("only one endpoint handler is allowed for a request, middleware handler must accept the argument ChainExecutor")
	}
...
func (ch *handlerChain) addEndpoint(h ...AnyHandler) *handlerChain {
	farmutil.MustTrue(ch.endpoint == nil, "one handler chain can only have one endpoint")
	for _, p := range h {
		switch h := p.(type) {
		case *handlerChain:
			ch.middlewares = append(ch.middlewares, h.middlewares...)
		case RequestHandler:
			ch.middlewares = append(ch.middlewares, NewHandlerCaller(h.HandlerFunc))
		default:
			ch.middlewares = append(ch.middlewares, NewHandlerCaller(p))
		}
	}
	cnt := len(ch.middlewares)
	farmutil.MustTrue(cnt != 0, "no endpoint handler")
	ch.endpoint = ch.middlewares[cnt-1]
	ch.middlewares = ch.middlewares[:cnt-1]
	return ch
}

func (ch *handlerChain) HandlerFunc(c *Context) Response {
	mr := &ChainExecutor{
		chain:   ch,
		context: c,
	}
