...
func (wr *ResponseCommon) RespondTo(w http.ResponseWriter) (int64, error) {
	if wr.redirect != nil {
		...
		if statusCode == 0 {
			farmutil.Panic("unknown redirect type: %T", wr.redirect)
		}
	...
	case farmutil.JsonDataProvider:
	...
	case error:
		...
		return wr.respondJson(farmutil.Map{"Error": v.Error()})
	default:
		if v != nil {
			farmutil.Panic("unknown response body: %T", v)
