...
func (c *Context) Respond(vars ...any) *ResponseCommon {
	...
	for _, v0 := range vars {
		...
		case RedirectSeeOther, RedirectTemporary, RedirectPermanent:
			wr.redirect = v
		default:
			farmutil.MustTrue(wr.respBody == nil, "response body was already set")
			wr.respBody = v
		}
	}
	return wr
...
func (c *Context) RespondRedirectSameSite(location string, params ...farmutil.Map) *ResponseCommon {
	isValid := false
	...
	if !isValid {
		farmlog.Debugf("location %s is not the same site as %s, redirection is denied", location, c.UriHost())
		return c.Respond(RedirectSeeOther("/"))
	}
	uri := farmutil.BuildUrl("", location, farmutil.DefZero(params))
	return c.Respond(RedirectSeeOther(uri))
}

func (c *Context) RespondJson(v any) *ResponseCommon {
	resp := c.Respond(farmutil.AsJsonDataProvider(v))
...
func (c *Context) UriHost() string {
	...
	schema = farmutil.IfZero(schema, "http")
...
func (c *Context) RealRemoteIp() string {
	...
	farmutil.MustTrue(remoteIp != nil, "remote addr must contain valid IP")
