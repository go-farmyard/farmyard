...
func (cb *CmdBuilder) Table(v any) *CmdBuilder {
	...
	default:
		farmutil.Panic("unsupported table name type: %T", v)
	}
	return cb
}

func (cb *CmdBuilder) setColumns(cols []string) {
	farmutil.MustTrue(cb.columns == nil, "columns can only be set once")
	cb.columns = cols
}
...
func toClauseProvider(clause any, args ...any) ClauseProvider {
	...
	case ClauseProvider:
		farmutil.MustTrue(len(args) == 0, "clause should not have args")
		return c
	case string:
		return CondArgs(c, args...)
	case TableNameProvider:
		farmutil.Panic("you should convert a model to a cond (CondModel)")
		return nil
	default:
		farmutil.Panic("unsupported clause type: %T", clause)
		return nil
	}
}
...
func (cb *CmdBuilder) Limit(n ...int) *CmdBuilder {
	if len(n) == 1 {
		...
		farmutil.Panic("invalid limit args")
	}
	return cb
}

func (cb *CmdBuilder) startCommand(cmd string) error {
	farmutil.MustTrue(cb.sqlCmdPrefix == "", "sql command should be only set once")
	cb.sqlCmdPrefix = cmd
...
func (cb *CmdBuilder) prepareTableRowByColumns(row any) ModelStructFields {
	...
	farmutil.MustTrue(cb.tableReference != "", "sql command lacks table reference")
	farmutil.MustTrue(len(cb.columns) != 0, "columns must be set")

	if len(cb.columns) == 1 {
		if cb.columns[0] == columnsFull[0] {
			fields, err := modelStructFieldValues(cb.orm.fieldMapper, row, true)
			farmutil.MustNoError(err, "row must be a map or model struct")
			return fields
	...
	farmutil.MustNoError(err, "can not get row columns")
	return fields
}
...
func (cb *CmdBuilder) prepareSelectQuery() error {
	...
	if len(cb.columns) != 0 {
		farmutil.MustTrue(!(len(cb.columns) == 1 && cb.columns[0] == columnsModel[0]), "select can not work with ColumnsModel")
